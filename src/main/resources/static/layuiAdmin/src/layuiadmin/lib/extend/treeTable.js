layui.define(function (exports) {
    ...
    tree.prototype.template = function (e) {
        ...
        o.each(t.data(e, e.data), function (idx, item) {
            ...
            // 设置每行数据层级
            item.level = level[item[e.id_key]] = item[e.parent_key] != e.top_value ? (level[item[e.parent_key]] + 1) : 0;
            // 设置是否为最后一级
            item.is_end = !e.childs[item[e.id_key]];
            o.each(e.cols, function (index, obj) {
                ...
                // 指定列加入开启、关闭小图标
                var icon = (obj.key == e.icon_key && !item.is_end) ? '<i class="' + (t.cache(e, item[e.id_key]) ? e.icon.open : e.icon.close) + '"></i>' : '<span></span>';
                // 指定列小图标按照层级向后位移
                var left = (obj.key == e.icon_key ? level[item[e.id_key]] * e.icon.left + 'px' : '');
            ...
            tbody += '<tr class="' + hide_class + '" data-idkey="' + item[e.primary_key] + '" data-id="' + item[e.id_key] + '" data-pid="' + item[e.parent_key] + '">' + box + tr + '</tr>';
        });
        // 处理表树和树的赋值模板
    ...
    tree.prototype.data = function (e) {
        ...
        o.each(e.data, function (idx, item) {
            lists[item[e.id_key]] = item;
            if (!childs[item[e.parent_key]]) {
                childs[item[e.parent_key]] = [];
            }
            childs[item[e.parent_key]][item[e.id_key]] = item;
        });
        e.childs = childs;
    ...
    tree.prototype.tree_data = function (e, lists, pid, data) {
        ...
        o.each(e.data, function (index, item) {
            if (item[e.parent_key] == pid) {
                data.concat(t.tree_data(e, lists, item[e.id_key], data))
            }
        });
        return data;
    };
    ...
    tree.prototype.checked = function (e) {
        ...
        o(e.elem).find('.cbx.layui-form-checked').each(function () {
            var id = o(this).parents('[data-idkey]').data('idkey');
            ids.push(id);
        });
        return ids;
    };
    ...
    tree.prototype.openAll = function (e) {
        var t = this;
        o.each(e.data, function (idx, item) {
            item[e.id_key] && t.cache(e, item[e.id_key], true);
        });
        t.render(e);
    };
    // 全部关闭
