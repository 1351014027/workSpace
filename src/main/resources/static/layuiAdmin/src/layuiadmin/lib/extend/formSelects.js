...
(function (layui, window, factory) {
    ...
    var v = '4.0.0.0910',
        ...
        FormSelects = function FormSelects(options) {
            ...
            this.config = {
                ...
                max: null,
                savingBtn: options.savingBtn || false,
    ...
    Common.prototype.ajax = function (id, searchUrl, inputValue, isLinkage, linkageWidth, isSearch, successCallback, isReplace) {
        ...
        $.ajax({
            ...
            url: searchUrl,
            data: {Data: (ajaxConfig.dataType == 'json' ? JSON.stringify(ajaxData) : ajaxData)},
            success: function success(res) {
                if (typeof res == 'string') {
                    res = JSON.parse(res);
                }
                res[ajaxConfig.response.dataName] = Object.values(res[ajaxConfig.response.dataName]);
    ...
    Common.prototype.renderSelect = function (id, tips, select) {
        ...
        arr.push('<dd class="' + FORM_SELECT_TIPS + ' ' + FORM_NONE + ' ' + (arr.length === 2 ? FORM_EMPTY : '') + '">\u6CA1\u6709\u9009\u9879' + (data[id].config.savingBtn ? '--是否<a class="saving" href="javascript:;" style="color: #1E9FFF"><i class="layui-icon layui-icon-add-1"></i><span>添加</span></a>' : '') + '</dd>');
        return arr.join('');
    };
    ...
    Select4.prototype.config = function (id, config, isJson) {
        ...
        if (config && (typeof config === 'undefined' ? 'undefined' : _typeof(config)) == 'object') {
            ...
            id ? (ajaxs[id] = $.extend(true, {}, ajaxs[id] || ajax, config), !common.check(id) && this.render(id), data[id] && config.direction && (data[id].config.direction = config.direction), data[id] && config.savingBtn && (data[id].config.savingBtn = config.savingBtn), data[id] && config.clearInput && (data[id].config.clearInput = true), config.searchUrl && data[id] && common.triggerSearch($('.' + PNAME + ' dl[xid="' + id + '"]').parents('.' + FORM_SELECT), true)) : ($.extend(true, ajax, config), $.each(ajaxs, function (key, item) {
