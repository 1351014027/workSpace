layui.define(['table', 'form'], function (exports) {
    ...
    Class.prototype.render = function () {
        ...
        var d1_c = {
            ...
            , parseData: function (res) {
                ...
                if (transfer.data.length < 2) {
                    transfer.data.push({id: LEFT_TABLE + that.index, data: (res.data ? res.data : [])})
                } else {
                    if (transfer.data[0].id.indexOf(LEFT_TABLE) >= 0) {
                        transfer.data[0].data = res.data
                    }
                }
            }
            //saving
    ...
    function datas(type) {
        var that = this;
        var d = transfer.data;
        var d1 = d[0].id.indexOf(LEFT_TABLE) >= 0 ? d[0].data : d[1].data;
        var d2 = d[0].id.indexOf(RIGHT_TABLE) >= 0 ? d[0].data : d[1].data;
        var _d = [];
    ...
    function uniq(array) {
        ...
        for (var i = 0; i < l; i++) {
            for (var j = i + 1; j < l; j++) {
                if (array[i].oneid === array[j].oneid) {
                    if (array[i].twoid === array[j].twoid) {
                        i++;
                        j = i;
                    }
                }
